    ...
    def __init__(self, env, always_keys=None, reverse_keys=None, exclude_keys=None, exclude_noop=False,
        ...
        for key in self.noop:
            ...
            if key in self.BINARY_KEYS:
                ...
                for i in range(self.num_camera_discretize + 1):
                    op = copy.deepcopy(self.noop)
                    if i < self.num_camera_discretize // 2:
                        if (-max_camera_range + delta_range * i == 0) and self.noop['forward'] == 1: # avoid redundant action for neutral camera rotation
                            continue
                        op[key] = np.array([0, -max_camera_range + delta_range * i], dtype=np.float32)
                    elif i > self.num_camera_discretize // 2:
                        if (-max_camera_range + delta_range * (i-1) == 0) and self.noop['forward'] == 1: # avoid redundant action for neutral camera rotation
                            continue
                ...
                if allow_pitch:
                    for i in range(self.num_camera_discretize + 1):
                        op = copy.deepcopy(self.noop)
                        if i < self.num_camera_discretize // 2:
                            # avoid redundant action for neutral camera rotation
                            if (-max_camera_range + delta_range * i == 0) and self.noop['forward'] == 1:
                                continue
                            op[key] = np.array([-max_camera_range + delta_range * i, 0], dtype=np.float32)
                        elif i > self.num_camera_discretize // 2:
                            # avoid redundant action for neutral camera rotation
                            if (-max_camera_range + delta_range * (i-1) == 0) and self.noop['forward'] == 1:
                                continue
